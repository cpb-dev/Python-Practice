...
def read_applications():
    no_applications = len(df.index)
    print("\n So far you have applied to ", no_applications, " jobs!")
    print(df.tail(10))
...
def search_roles(role):
    result = df.loc[df['role'].str.contains(role)] #Means if the role contains at least one similar word
    print(result)
